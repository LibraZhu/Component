...
import com.libra.indicator.abs.IMeasurablePagerTitleView;
import com.libra.indicator.abs.IPagerIndicator;
import com.libra.indicator.abs.IPagerNavigator;
import com.libra.indicator.abs.IPagerTitleView;
import com.libra.indicator.abs.ScrollState;
import com.libra.indicator.helper.NavigatorHelper;
...
public class CommonNavigator extends FrameLayout
    ...
    private boolean mAdjustModeWeight;
    ...
    private DataSetObserver mObserver = new DataSetObserver() {
        ...
        public void onChanged() {
            mNavigatorHelper
                    .setTotalCount(mAdapter.getCount());    // 如果使用helper，应始终保证helper中的totalCount为最新
            init();
        }
    ...
    public void setmAdjustModeWeight(boolean mAdjustModeWeight) {
        this.mAdjustModeWeight = mAdjustModeWeight;
    }
    ...
    public void setAdapter(CommonNavigatorAdapter adapter) {
        ...
        if (mAdapter != null) {
            ...
            if (mTitleContainer
                    != null) {  // adapter改变时，应该重新init，但是第一次设置adapter不用，onAttachToMagicIndicator中有init
                mAdapter.notifyDataSetChanged();
            }
        } else {
    ...
    private void init() {
        ...
        if (mAdjustMode) {
            root = LayoutInflater.from(getContext())
                                 .inflate(R.layout.layout_pager_navigator_no_scroll, this);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mAdapter != null) {
            ...
            if (mReselectWhenLayout
                    && mNavigatorHelper.getScrollState() == ScrollState.SCROLL_STATE_IDLE) {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if (mAdapter != null) {
            ...
            // 手指跟随滚动
            if (mScrollView != null
                    && mPositionDataList.size() > 0
                    && position >= 0
                    && position < mPositionDataList.size()) {
                if (mFollowTouch) {
                    ...
                    float scrollTo =
                            current.horizontalCenter() - mScrollView.getWidth() * mScrollPivotX;
                    float nextScrollTo =
                            next.horizontalCenter() - mScrollView.getWidth() * mScrollPivotX;
                    mScrollView
                            .scrollTo((int) (scrollTo + (nextScrollTo - scrollTo) * positionOffset),
                                      0);
