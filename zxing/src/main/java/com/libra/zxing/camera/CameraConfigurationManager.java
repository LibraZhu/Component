...
package com.libra.zxing.camera;
...
final class CameraConfigurationManager {
    ...
    private static Point getCameraResolution(Camera.Parameters parameters, Point screenResolution) {
        ...
        if (cameraResolution == null) {
            ...
            cameraResolution =
                    new Point((screenResolution.x >> 3) << 3, (screenResolution.y >> 3) << 3);
        }
    ...
    void initFromCameraParameters(Camera camera) {
        ...
        mCameraResolution = findCloselySize(ScreenUtils.getScreenWidth(context),
                                            ScreenUtils.getScreenHeight(context),
                                            parameters.getSupportedPreviewSizes());
        Log.e(TAG,
              "Setting preview size: " + mCameraResolution.width + "-" + mCameraResolution.height);
        mPictureResolution = findCloselySize(ScreenUtils.getScreenWidth(context),
                                             ScreenUtils.getScreenHeight(context),
                                             parameters.getSupportedPictureSizes());
        Log.e(TAG, "Setting picture size: "
                + mPictureResolution.width
                + "-"
                + mPictureResolution.height);
    }

    /**
     * Sets the camera up to take preview images which are used for both preview and decoding. We
     * detect the preview
     * format here so that buildLuminanceSource() can build an appropriate LuminanceSource subclass.
     * In the future we
     * may want to force YUV420SP as it's the smallest, and the planar Y can be used for barcode
     * scanning without a copy
     * in some cases.
     */
     ...
     */
    protected Camera.Size findCloselySize(int surfaceWidth, int surfaceHeight,
