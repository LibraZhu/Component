...
import com.libra.zxing.R;
import com.libra.zxing.utils.ScreenUtils;
...
public final class QrCodeFinderView extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        RelativeLayout relativeLayout =
                (RelativeLayout) inflater.inflate(R.layout.layout_qr_code_scanner, this);

        FrameLayout frameLayout =
                (FrameLayout) relativeLayout.findViewById(R.id.qr_code_fl_scanner);
        mFrameRect = new Rect();
    ...
    private void getCustomFrameRect() {
        mFrameRect.top = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 260,
    ...
    private void drawFocusRect(Canvas canvas, Rect rect) {
        ...
        // 上
        canvas.drawRect(rect.left + mAngleLength, rect.top, rect.right - mAngleLength,
                        rect.top + mFocusThick, mPaint);
        // 左
        canvas.drawRect(rect.left, rect.top + mAngleLength, rect.left + mFocusThick,
                        rect.bottom - mAngleLength, mPaint);
        // 右
        canvas.drawRect(rect.right - mFocusThick, rect.top + mAngleLength, rect.right,
                        rect.bottom - mAngleLength, mPaint);
        // 下
        canvas.drawRect(rect.left + mAngleLength, rect.bottom - mFocusThick,
                        rect.right - mAngleLength, rect.bottom, mPaint);
    }
    ...
    private void drawText(Canvas canvas, Rect rect) {
        ...
        float left = (ScreenUtils.getScreenWidth(mContext) - getTextWidth(mPaint, text)) / 2;
